...
static int replaceFunc(void *addr, void *replace_func, void **old_func){
	int res = 0;
	//LOGD("replaceFunc:addr=%08x, replace_func=%08x, *old_func=%08x, *addr=%08x", (uint32_t)addr, (uint32_t)replace_func, *(uint32_t *)addr, (uint32_t)old_func);
	...
	*(void **)addr = replace_func;
	...
	//LOGD("[+] old_func is %p, addr is %p, new_func %p.", *old_func, (uint32_t)addr, *(uint32_t *)addr);
...
int elfHook(const char *soname, const char *symbol, void *replace_func, void **old_func){
	...
	assert(replace_func);
	assert(symbol);
        //LOGD("elfHook:addr=%08x, replace_func=%08x, *old_func=%08x, old_func=%08x", (uint32_t)addr, (uint32_t)replace_func, (uint32_t)*old_func, (uint32_t)old_func);
	

	ElfHandle tHandle = {0};
	ElfHandle *handle = &tHandle;
	if ( !openElfBySoname(soname, handle) ) {
		return 0;
	}
	ElfInfo info;
