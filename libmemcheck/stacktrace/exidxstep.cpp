...
int arm_exidx_extract (t_cursor *c, uint8_t *buf) {
   ...
   */
  uint32_t key_value_addr = search_exidx_value((uint32_t *)c->exidx_start_addr, (uint32_t *)c->exidx_end_addr, c->base_addr,c->pc_offest);
  if(key_value_addr == 0) {
    /* 找不到exidx数据，终止回溯 */
...
uint32_t search_exidx_value(uint32_t *start, uint32_t *end, uint32_t base_addr, uint32_t key) {
  ...
  /*一个key-value是64bits*/
  uint32_t *mid = start + ((((uint32_t)end - (uint32_t)start)/8)/2)*2;
  /*取前32bits*/
  uint32_t key_pre = (*(mid-2) + (uint32_t)(mid-2) - base_addr) & 0x7FFFFFFF;
  uint32_t key_mid = (*mid + (uint32_t)mid - base_addr) & 0x7FFFFFFF;
  uint32_t key_flo = (*(mid+2) + (uint32_t)(mid+2) - base_addr) & 0x7FFFFFFF;
  if(key >= key_mid && key <= key_flo) {
    return (uint32_t )mid;
  }else if(key <= key_mid && key >= key_pre) {
    return (uint32_t )(mid - 2);
  }else if(key >= key_mid) {
...
int copyregs_to_cursor(sigcontext *sig_ctx, t_cursor *c) {
  c->regs[ARM_R0] = sig_ctx->arm_r0;
  c->regs[ARM_R1] = sig_ctx->arm_r1;
  c->regs[ARM_R2] = sig_ctx->arm_r2;
  c->regs[ARM_R3] = sig_ctx->arm_r3;
  c->regs[ARM_R4] = sig_ctx->arm_r4;
  c->regs[ARM_R5] = sig_ctx->arm_r5;
  c->regs[ARM_R6] = sig_ctx->arm_r6;
  c->regs[ARM_R7] = sig_ctx->arm_r7;
  c->regs[ARM_R8] = sig_ctx->arm_r8;
  c->regs[ARM_R9] = sig_ctx->arm_r9;
  c->regs[ARM_SL] = sig_ctx->arm_r10;
  c->regs[ARM_FP] = sig_ctx->arm_fp;
  c->regs[ARM_IP] = sig_ctx->arm_ip;
  c->regs[ARM_SP] = sig_ctx->arm_sp;
  c->regs[ARM_LR] = sig_ctx->arm_lr;
  c->regs[ARM_PC] = sig_ctx->arm_pc;
  return 0;
}

int copyregs_to_sigctx(t_cursor *c, sigcontext *sig_ctx) {
  sig_ctx->arm_r0 = c->regs[ARM_R0];
  sig_ctx->arm_r1 = c->regs[ARM_R1];
  sig_ctx->arm_r2 = c->regs[ARM_R2];
  sig_ctx->arm_r3 = c->regs[ARM_R3];
  sig_ctx->arm_r4 = c->regs[ARM_R4];
  sig_ctx->arm_r5 = c->regs[ARM_R5];
  sig_ctx->arm_r6 = c->regs[ARM_R6];
  sig_ctx->arm_r7 = c->regs[ARM_R7];
  sig_ctx->arm_r8 = c->regs[ARM_R8];
  sig_ctx->arm_r9 = c->regs[ARM_R9];
  sig_ctx->arm_r10 = c->regs[ARM_SL];
  sig_ctx->arm_fp = c->regs[ARM_FP];
  sig_ctx->arm_ip = c->regs[ARM_IP];
  sig_ctx->arm_sp = c->regs[ARM_SP];
  sig_ctx->arm_lr = c->regs[ARM_LR];
  sig_ctx->arm_pc = c->regs[ARM_PC];
  return 0;
}
